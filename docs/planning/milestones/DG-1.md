...
Owner: DevGuard. Current implementation: `implemented` (C01–C12); qualification: macOS `qualified` for release `0.1.0-5daee5d-b3fa569e` on the measured host and policy, Linux `not-run`. Entry: DG-0. Completion: DG1-C12 qualifies actual macOS launch/reconciliation, generic/Cargo consumption, parent-budget self-use, independent repair and development/foreground SLO for the measured artifact/policy/environment.
...
Available regression: `python3 scripts/validate.py --offline` (Rust 1.95.0 contract regression; fake backends do not prove native behavior). C01 authority, C02 authentication/transport, C03 native probe, C04 native scope, C05 native launch, C06 native reconciliation, C07 CLI, C08 Cargo, C09 installation, C10 parent-lease, C11 upgrade and C12 SLO harness suites are now available; commands explicitly labelled planned below remain unavailable until implemented. Each PR must supply real fixtures, nonzero case counts, logs and cleanup, then update command availability. See [verification](../verification.md).
...
- Verification command: available regression above plus **available** `python3 scripts/qualify.py dg1-upgrade --offline` (macOS only; other platforms record `not_run`). It replaces fixture releases through a fake service manager; the real upgrade of the installed service is recorded separately as completion evidence.
...
- Verification command: available regression above plus **available** `python3 scripts/qualify.py dg1-macos --offline`. It is macOS only; other platforms record `not_run`. It checks the harness, never the SLO. The SLO protocol itself is **available** `python3 scripts/measure.py macos --release ID ...`, run against the installed release on the target host in a window set aside for it; its verdicts and any promotion are recorded as completion evidence.
- Result: protocol-2, run on 2026-09-25 and 26 with harness `72c4220`, measured release `0.1.0-5daee5d-b3fa569e`. Both combinations passed all three repetitions. Input to next paint had p99 at most 56 ms, status p99 at most 47 ms and termination acknowledgement p99 at most 67 ms. No frame stalled, and all 1,408 load runs succeeded without connection loss, uncertain execution or duplicate launch. `measure.py promote` recorded the qualification after verifying that the service runs that release. The first run, protocol-1, failed on the harness's own evidence writes; it is preserved with its analysis.

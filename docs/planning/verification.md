...
C08 evidence covers:
- **Raw receipts**: a direct build within its reserved jobs; explicit jobs clamped and kept; refusals before admission; a pipeline's shared jobserver with its tokens after the run; nested Cargo in a build script; an inherited jobserver; stale inherited descriptors; concurrent consumers with their peak charges; and a cancelled pipeline. Each records the observed peak of concurrent compilations.
...
- **Unit tests**: the job estimate; argument parsing around the subcommand and `--`; precedence, clamping and refusals; inherited-jobserver checks for closed, close-on-exec and FIFO references; the FIFO pool and its token count; and adapter selection.

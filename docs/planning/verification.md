...
- **Clamped environment**: when the environment clamps every child of the harness, as governed self-use will, the unclamped-root case cannot be produced. It is recorded as `not_run` with both readbacks, and the suite reports `incomplete` rather than `passed`.

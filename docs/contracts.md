...
This document describes the implemented DG-0 authority, C01/C02 service, storage and transport behavior, C03 native macOS host evidence, C04 cooperative policy and scope evidence and the C05 fenced launch helper. PR and post-merge main delivery evidence is tracked separately from implementation. [Operations](operations.md) lists actual command availability. The normal service keeps registration and launch closed until reconciliation ships with them, and CodeSpace integration remains unimplemented. [Korean translation](ko/contracts.md).
...
- **Release.** Release still requires the core's full evidence. A reaped root with surviving descendants, a zombie member, a known escape and incomplete tracking each prevent it. An unbound committed launch cannot be released through this backend until DG1-C06 supplies evidence that no helper was created.
- **Limits.** The authority process holds scope tracking in memory. After a restart, previously bound scopes are unknown to the backend, so their attempts become Suspect until explicit reconciliation (DG1-C06). A member the authority cannot observe, such as a setuid program, causes permanent tracking loss for its scope. If the root is reaped while no known member remains in the group, surviving unknown members cannot be proven to be the scope's. They are permanent tracking loss and are not signalled, so a scope must be observed before its root is reaped (DG1-C06).

## Fenced launch helper

DG1-C05 adds the launch helper, `devguard-launch`, and the wire requests that lead to it. The normal service keeps registration and launch closed until DG1-C06 ships reconciliation with them; in C05 only isolated test authorities open them.
...
- Remaining DG-1: reconciliation, including evidence that no helper was created; opening registration and launch in the normal service; the execution CLI, Cargo adaptation, bounded self-use, update/repair and measured macOS SLOs.

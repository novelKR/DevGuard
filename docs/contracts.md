...
This document describes the implemented DG-0 authority, C01/C02 service, storage and transport behavior, and C03 native macOS host evidence. PR and post-merge main delivery evidence is tracked separately from implementation. [Operations](operations.md) lists actual command availability; registration over the wire, launch, applied OS policies and CodeSpace integration remain unimplemented. [Korean translation](ko/contracts.md).
...
- Remaining DG-1: registration over the wire, applied resource policy and scope evidence, launch helper, execution CLI, Cargo adaptation, bounded self-use, update/repair and measured macOS SLOs.

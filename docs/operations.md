...
- **Output.** Raw samples, receipts and a `report.json` per repetition go under `--out`, written by a writer thread. Sources, Cargo targets and the I/O workload's files go under `--work`, which can be removed afterwards. The probe's binary and the browser profile run from a temporary stage under `/private/tmp` on the internal disk, which is removed at the end. `summary.json` gives each combination's verdict, the overall one, and each target's status (measured, not applicable or not run).

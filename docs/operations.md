...
- `test-candidate` running a workload and a candidate as children of one lease that is then released, with the candidate's area removed, and a candidate that dies before serving, which fails the run while its scope and the lease are still released
...
- repair refused while an authority serves, using the recovery copy of a damaged release, and keeping a journal that cannot be opened closed
